...
#[doc = "Field `STARTMSK_L` reader - If bit n is one, event n clears the CTRL\\[STOP_L\\]
= 0 (event 0 = bit 0, event 1 = bit 1, etc.). The number of bits = number of events in this SCT."]
pub type STARTMSK_L_R = crate::FieldReader<u16, u16>;
#[doc = "Field `STARTMSK_L` writer - If bit n is one, event n clears the CTRL\\[STOP_L\\]
= 0 (event 0 = bit 0, event 1 = bit 1, etc.). The number of bits = number of events in this SCT."]
pub type STARTMSK_L_W<'a, const O: u8> = crate::FieldWriter<'a, u32, START_SPEC, u16, u16, 16, O>;
#[doc = "Field `STARTMSK_H` reader - If bit n is one, event n clears the CTRL\\[STOP_H\\]
= 0 (event 0 = bit 16, event 1 = bit 17, etc.). The number of bits = number of events in this SCT."]
pub type STARTMSK_H_R = crate::FieldReader<u16, u16>;
#[doc = "Field `STARTMSK_H` writer - If bit n is one, event n clears the CTRL\\[STOP_H\\]
= 0 (event 0 = bit 16, event 1 = bit 17, etc.). The number of bits = number of events in this SCT."]
pub type STARTMSK_H_W<'a, const O: u8> = crate::FieldWriter<'a, u32, START_SPEC, u16, u16, 16, O>;
impl R {
    #[doc = "Bits 0:15 - If bit n is one, event n clears the CTRL\\[STOP_L\\]
= 0 (event 0 = bit 0, event 1 = bit 1, etc.). The number of bits = number of events in this SCT."]
    #[inline(always)]
    pub fn startmsk_l(&self) -> STARTMSK_L_R {
        STARTMSK_L_R::new((self.bits & 0xffff) as u16)
    }
    #[doc = "Bits 16:31 - If bit n is one, event n clears the CTRL\\[STOP_H\\]
= 0 (event 0 = bit 16, event 1 = bit 17, etc.). The number of bits = number of events in this SCT."]
    #[inline(always)]
